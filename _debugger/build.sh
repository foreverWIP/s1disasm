...
dd ibs=1 skip=$((JumpSkip          )) count=$((HackerName-JumpSkip                 )) if=blob.bin of=../_mcd/src/debuggerblob/Part1.bin status=none
dd ibs=1 skip=$((BusErrorMsg       )) count=$((BusError-BusErrorMsg                )) if=blob.bin of=../_mcd/src/debuggerblob/Part2.bin status=none
dd ibs=1 skip=$((BusError          )) count=$((AddressError-BusError               )) if=blob.bin of=../_mcd/src/debuggerblob/Part3.bin status=none
dd ibs=1 skip=$((AddressError      )) count=$((TraceError-AddressError             )) if=blob.bin of=../_mcd/src/debuggerblob/Part4.bin status=none
dd ibs=1 skip=$((TraceError        )) count=$((SpuriousException-TraceError        )) if=blob.bin of=../_mcd/src/debuggerblob/Part5.bin status=none
dd ibs=1 skip=$((SpuriousException )) count=$((ZeroDivideError-SpuriousException   )) if=blob.bin of=../_mcd/src/debuggerblob/Part6.bin status=none
dd ibs=1 skip=$((ZeroDivideError   )) count=$((CHKExceptionError-ZeroDivideError   )) if=blob.bin of=../_mcd/src/debuggerblob/Part7.bin status=none
dd ibs=1 skip=$((CHKExceptionError )) count=$((TRAPVError-CHKExceptionError        )) if=blob.bin of=../_mcd/src/debuggerblob/Part8.bin status=none
dd ibs=1 skip=$((TRAPVError        )) count=$((IllegalInstrError-TRAPVError        )) if=blob.bin of=../_mcd/src/debuggerblob/Part9.bin status=none
dd ibs=1 skip=$((IllegalInstrError )) count=$((PrivilegeViolation-IllegalInstrError)) if=blob.bin of=../_mcd/src/debuggerblob/PartA.bin status=none
dd ibs=1 skip=$((PrivilegeViolation)) count=$((LineAEmulation-PrivilegeViolation   )) if=blob.bin of=../_mcd/src/debuggerblob/PartB.bin status=none
dd ibs=1 skip=$((LineAEmulation    )) count=$((LineFEmulation-LineAEmulation       )) if=blob.bin of=../_mcd/src/debuggerblob/PartC.bin status=none
dd ibs=1 skip=$((LineFEmulation    )) count=$((TrapVector-LineFEmulation           )) if=blob.bin of=../_mcd/src/debuggerblob/PartD.bin status=none
dd ibs=1 skip=$((TrapVector        )) count=$((post_revision+2-TrapVector          )) if=blob.bin of=../_mcd/src/debuggerblob/PartE.bin status=none
dd ibs=1 skip=$((post_revision+4   )) count=$((padding_start-(post_revision+4)     )) if=blob.bin of=../_mcd/src/debuggerblob/PartF.bin status=none
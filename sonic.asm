...
StartOfRom:
		if NeoGeo<>1
		include "headermd.asm"
		else
		include "ngregdefs.asm"
		include "headerng.asm"
		endif
EndOfHeader:
...
.clearRAM:
		...
		bsr.w	DACDriverLoad
		bsr.w	JoypadInit
		move.b	#id_Sega,v_gamemode ; set Game Mode to Sega Screen

MainGameLoop:
		move.b	v_gamemode,d0 ; load Game Mode
...
BusError:
		move.b	#2,v_errortype
		bra.s	loc_43A

AddressError:
		move.b	#4,v_errortype
		bra.s	loc_43A
...
ChkInstr:
		move.b	#$A,v_errortype
		bra.s	loc_462

TrapvInstr:
		move.b	#$C,v_errortype
		bra.s	loc_462

PrivilegeViol:
		move.b	#$E,v_errortype
		bra.s	loc_462

Trace:
		move.b	#$10,v_errortype
		bra.s	loc_462

Line1010Emu:
		move.b	#$12,v_errortype
		addq.l	#2,2(sp)
		bra.s	loc_462

Line1111Emu:
		move.b	#$14,v_errortype
		addq.l	#2,2(sp)
		bra.s	loc_462

ErrorExcept:
		move.b	#0,v_errortype
		bra.s	loc_462
; ===========================================================================
...
loc_43A:
		disable_ints
		addq.w	#2,sp
		move.l	(sp)+,v_spbuffer
		addq.w	#2,sp
		movem.l	d0-a7,v_regbuffer
		bsr.w	ShowErrorMessage
		move.l	2(sp),d0
		bsr.w	ShowErrorValue
		move.l	v_spbuffer,d0
		bsr.w	ShowErrorValue
		bra.s	loc_478
; ===========================================================================
...
.loadgfx:
		...
		moveq	#0,d0		; clear	d0
		move.b	v_errortype,d0 ; load error code
...
ErrorWaitForC:
		bsr.w	ReadJoypads
		cmpi.b	#btnC,v_jpadpress1 ; is button C pressed?
...
.notPAL:
		move.b	v_vbla_routine,d0
		move.b	#0,v_vbla_routine
		move.w	#1,f_hbla_pal
		andi.w	#$3E,d0
...
VBla_00:
		cmpi.b	#$80+id_Level,v_gamemode
		beq.s	.islevel
		cmpi.b	#id_Level,v_gamemode ; is game on a level?
...
.notPAL:
		move.w	#1,f_hbla_pal ; set HBlank flag
		stopZ80
		waitZ80
		tst.b	f_wtr_state	; is water above top of screen?
...
VBla_10:
		cmpi.b	#id_Special,v_gamemode ; is game on special stage?
...
.waterbelow:
		...
		tst.b	f_sonframechg ; has Sonic's sprite changed?
		beq.s	.nochg		; if not, branch

		writeVRAM	v_sgfx_buffer,$2E0,ArtTile_Sonic*$20 ; load new Sonic gfx
		move.b	#0,f_sonframechg

.nochg:
		startZ80
		movem.l	v_screenposx,d0-d7
		movem.l	d0-d7,v_screenposx_dup
		movem.l	v_fg_scroll_flags,d0-d1
		movem.l	d0-d1,v_fg_scroll_flags_dup
		cmpi.b	#96,v_hbla_line
		bhs.s	Demo_Time
		move.b	#1,f_doupdatesinhblank
		addq.l	#4,sp
		bra.w	VBla_Exit
...
Demo_Time:
		...
		jsr	(HUD_Update).l
		bsr.w	ProcessDPLC2
		tst.w	v_demolength ; is there time left on the demo?
		beq.w	.end		; if not, branch
		subq.w	#1,v_demolength ; subtract 1 from time left
...
.nochg:
		...
		subq.w	#1,v_demolength	; subtract 1 from time left in demo
...
VBla_0E:
		bsr.w	sub_106E
		addq.b	#1,v_vbla_0e_counter ; Unused besides this one write...
		move.b	#$E,v_vbla_routine
		rts	
; ===========================================================================
...
HBlank:
		disable_ints
		tst.w	f_hbla_pal	; is palette set to change?
		beq.s	.nochg		; if not, branch
		move.w	#0,f_hbla_pal
		movem.l	a0-a1,-(sp)
		if NeoGeo<>1
...
.setreg:
		...
		move.w	d0,v_vdp_buffer1
		move.w	#$8A00+223,v_hbla_hreg	; H-INT every 224th scanline
		moveq	#0,d0
		move.l	#$C0000000,(vdp_control_port).l ; set VDP to CRAM write
		move.w	#$3F,d7
...
ClearScreen:
		...
		if Revision=0
		move.l	#0,v_scrposy_vdp
		move.l	#0,v_scrposx_vdp
		else
		clr.l	v_scrposy_vdp
		clr.l	v_scrposx_vdp
		endif
...
AddPLC:
		...
		lea	v_plc_buffer,a2 ; PLC buffer space
...
ClearPLC:
		lea	v_plc_buffer,a2 ; PLC buffer space in RAM
...
RunPLC:
		...
		bne.s	Rplc_Exit
		movea.l	v_plc_buffer,a0
		lea	(NemPCD_WriteRowToVDP).l,a3
		lea	v_ngfx_buffer,a1
		move.w	(a0)+,d2
		bpl.s	loc_160E
		adda.w	#$A,a3
...
loc_160E:
		...
		moveq	#$10,d6
		moveq	#0,d0
		move.l	a0,v_plc_buffer
		move.l	a3,v_plc_ptrnemcode
		move.l	d0,v_plc_repeatcount
		move.l	d0,v_plc_paletteindex
		move.l	d0,v_plc_previousrow
...
sub_1642:
		tst.w	v_plc_patternsleft
		beq.w	locret_16DA
		move.w	#9,v_plc_framepatternsleft
		moveq	#0,d0
...
loc_1676:
		...
		subq.w	#4,a4
		movea.l	v_plc_buffer,a0
		movea.l	v_plc_ptrnemcode,a3
		move.l	v_plc_repeatcount,d0
		move.l	v_plc_paletteindex,d1
		move.l	v_plc_previousrow,d2
		move.l	v_plc_dataword,d5
		move.l	v_plc_shiftvalue,d6
		lea	v_ngfx_buffer,a1

loc_16AA:
		movea.w	#8,a5
		bsr.w	NemPCD_NewRow
		subq.w	#1,v_plc_patternsleft
		beq.s	loc_16DC
		subq.w	#1,v_plc_framepatternsleft
		bne.s	loc_16AA
		move.l	a0,v_plc_buffer
		move.l	a3,v_plc_ptrnemcode
		move.l	d0,v_plc_repeatcount
		move.l	d1,v_plc_paletteindex
...
PaletteFadeIn:
		move.w	#$003F,v_pfade_start ; set start position = 0; size = $40

PalFadeIn_Alt:				; start position and size are already set
		moveq	#0,d0
		lea	v_pal_dry,a0
		move.b	v_pfade_start,d0
		adda.w	d0,a0
		moveq	#cBlack,d1
		move.b	v_pfade_size,d0
...
.addcolour:
		...
		cmpi.b	#id_LZ,v_zone	; is level Labyrinth?
		bne.s	.exit		; if not, branch

		moveq	#0,d0
		lea	v_pal_water,a0
		lea	v_pal_water_dup,a1
		move.b	v_pfade_start,d0
		adda.w	d0,a0
		adda.w	d0,a1
		move.b	v_pfade_size,d0
...
PaletteFadeOut:
		move.w	#$003F,v_pfade_start ; start position = 0; size = $40
		move.w	#$15,d4
...
loc_2054:
		cmpi.w	#$64,d0
		blt.s	loc_2062
		move.w	#$401,v_pcyc_time
		moveq	#-$C,d0

loc_2062:
		move.w	d0,v_pcyc_num
		moveq	#1,d0
		rts	
; ===========================================================================
...
.wait:
		tst.b	v_vbla_routine ; has VBlank routine finished?
...
Sega_WaitPal:
		...
		move.b	#$14,v_vbla_routine
		bsr.w	WaitForVBla
		move.w	#$1E,v_demolength
...
Sega_WaitEnd:
		...
		andi.b	#btnStart,v_jpadpress1 ; is Start button pressed?
		beq.s	Sega_WaitEnd	; if not, branch

Sega_GotoTitle:
		move.b	#id_Title,v_gamemode ; go to title screen
		rts	
; ===========================================================================
...
GM_Title:
		...
		bsr.w	PalLoad1
		move.b	#id_CreditsText,v_sonicteam ; load "SONIC TEAM PRESENTS" object
...
Tit_LoadText:
		...
		move.w	#0,f_demo	; disable debug mode
		move.w	#0,v_unused2 ; unused variable
		move.w	#(id_GHZ<<8),v_zone	; set level to GHZ (00)
		move.w	#0,v_pcyc_time ; disable palette cycling
		bsr.w	LevelSizeLoad
		bsr.w	DeformLayers
		lea	v_16x16,a1
		...
		move.b	#0,f_debugmode ; disable debug mode
		move.w	#$178,v_demolength ; run title screen for $178 frames
	...
	if FixBugs
		...
		move.b	#id_TitleSonic,v_titlesonic ; load big Sonic object
		move.b	#id_PSBTM,v_pressstart ; load "PRESS START BUTTON" object
		;clr.b	v_pressstart+obRoutine ; The 'Mega Games 10' version of Sonic 1 added this line, to fix the 'PRESS START BUTTON' object not appearing
		...
		move.b	#id_PSBTM,v_titletm ; load "TM" object
		move.b	#3,v_titletm+obFrame
.isjap:
		move.b	#id_PSBTM,v_ttlsonichide ; load object which hides part of Sonic
		move.b	#2,v_ttlsonichide+obFrame
		jsr	(ExecuteObjects).l
		bsr.w	DeformLayers
		jsr	(BuildSprites).l
		moveq	#plcid_Main,d0
...
Tit_MainLoop:
		...
		move.b	#id_Sega,v_gamemode ; go to Sega screen
		rts	
; ===========================================================================

Tit_ChkRegion:
		tst.b	v_megadrive	; check	if the machine is US or	Japanese
...
Tit_EnterCheat:
		move.w	v_title_dcount,d0
		adda.w	d0,a0
		move.b	v_jpadpress1,d0 ; get button press
		andi.b	#btnDir,d0	; read only UDLR buttons
		cmp.b	(a0),d0		; does button press match the cheat code?
		bne.s	Tit_ResetCheat	; if not, branch
		addq.w	#1,v_title_dcount ; next button press
		tst.b	d0
		bne.s	Tit_CountC
		lea	f_levselcheat,a0
		move.w	v_title_ccount,d1
		lsr.w	#1,d1
		andi.w	#3,d1
		beq.s	Tit_PlayRing
		tst.b	v_megadrive
...
Tit_ResetCheat:
		tst.b	d0
		beq.s	Tit_CountC
		cmpi.w	#9,v_title_dcount
		beq.s	Tit_CountC
		move.w	#0,v_title_dcount ; reset UDLR counter

Tit_CountC:
		move.b	v_jpadpress1,d0
		andi.b	#btnC,d0	; is C button pressed?
		beq.s	loc_3230	; if not, branch
		addq.w	#1,v_title_ccount ; increment C counter

loc_3230:
		tst.w	v_demolength
		beq.w	GotoDemo
		andi.b	#btnStart,v_jpadpress1 ; check if Start is pressed
		beq.w	Tit_MainLoop	; if not, branch

Tit_ChkLevSel:
		tst.b	f_levselcheat ; check if level select code is on
		beq.w	PlayLevel	; if not, play level
		btst	#bitA,v_jpadhold1 ; check if A is pressed
		beq.w	PlayLevel	; if not, play level
...
LevelSelect:
		...
		move.w	v_levselsound,d0
		addi.w	#$80,d0
		tst.b	f_creditscheat ; is Japanese Credits cheat on?
...
LevSel_Ending:
		move.b	#id_Ending,v_gamemode ; set screen mode to $18 (Ending)
		move.w	#(id_EndZ<<8),v_zone ; set level to 0600 (Ending)
		rts	
; ===========================================================================

LevSel_Credits:
		move.b	#id_Credits,v_gamemode ; set screen mode to $1C (Credits)
		move.b	#bgm_Credits,d0
		bsr.w	PlaySound_Special ; play credits music
		move.w	#0,v_creditsnum
		rts	
; ===========================================================================
...
LevSel_Level_SS:
		...
		move.b	#id_Special,v_gamemode ; set screen mode to $10 (Special Stage)
		clr.w	v_zone	; clear	level
		move.b	#3,v_lives	; set lives to 3
		moveq	#0,d0
		move.w	d0,v_rings	; clear rings
		move.l	d0,v_time	; clear time
		move.l	d0,v_score	; clear score
		if Revision<>0
			move.l	#5000,v_scorelife ; extra life is awarded at 50000 points
		endif
		rts	
; ===========================================================================
...
LevSel_Level:
		andi.w	#$3FFF,d0
		move.w	d0,v_zone	; set level number

PlayLevel:
		move.b	#id_Level,v_gamemode ; set screen mode to $0C (level)
		move.b	#3,v_lives	; set lives to 3
		moveq	#0,d0
		move.w	d0,v_rings	; clear rings
		move.l	d0,v_time	; clear time
		move.l	d0,v_score	; clear score
		move.b	d0,v_lastspecial ; clear special stage number
		move.b	d0,v_emeralds ; clear emeralds
		move.l	d0,v_emldlist ; clear emeralds
		move.l	d0,v_emldlist+4 ; clear emeralds
		move.b	d0,v_continues ; clear continues
		if Revision<>0
			move.l	#5000,v_scorelife ; extra life is awarded at 50000 points
		endif
		move.b	#bgm_Fade,d0
...
loc_33E4:
		...
		move.b	#bgm_Fade,d0
		bsr.w	PlaySound_Special ; fade out music
		move.w	v_demonum,d0 ; load	demo number
		andi.w	#7,d0
		add.w	d0,d0
		move.w	Demo_Levels(pc,d0.w),d0	; load level number for	demo
		move.w	d0,v_zone
		addq.w	#1,v_demonum ; add 1 to demo number
		cmpi.w	#4,v_demonum ; is demo number less than 4?
		blo.s	loc_3422	; if yes, branch
		move.w	#0,v_demonum ; reset demo number to	0

loc_3422:
		move.w	#1,f_demo	; turn demo mode on
		move.b	#id_Demo,v_gamemode ; set screen mode to 08 (demo)
		cmpi.w	#$600,d0	; is level number 0600 (special	stage)?
		bne.s	Demo_Level	; if not, branch
		move.b	#id_Special,v_gamemode ; set screen mode to $10 (Special Stage)
		clr.w	v_zone	; clear	level number
		clr.b	v_lastspecial ; clear special stage number
...
LevSelControls:
		move.b	v_jpadpress1,d1
		andi.b	#btnUp+btnDn,d1	; is up/down pressed and held?
		bne.s	LevSel_UpDown	; if yes, branch
		subq.w	#1,v_levseldelay ; subtract 1 from time to next move
		bpl.s	LevSel_SndTest	; if time remains, branch

LevSel_UpDown:
		move.w	#$B,v_levseldelay ; reset time delay
...
LevSel_Refresh:
		move.w	d0,v_levselitem ; set new selection
		bsr.w	LevSelTextLoad	; refresh text
		rts	
; ===========================================================================

LevSel_SndTest:
		cmpi.w	#$14,v_levselitem ; is item $14 selected?
...
LevSel_Refresh2:
		move.w	d0,v_levselsound ; set sound test number
		bsr.w	LevSelTextLoad	; refresh text
...
GM_Level:
		bset	#7,v_gamemode ; add $80 to screen mode (for pre level sequence)
		tst.w	f_demo
...
Level_ClrRam:
		...
		move.w	(a1,d0.w),d0
		move.w	d0,v_waterpos1 ; set water heights
		move.w	d0,v_waterpos2
		move.w	d0,v_waterpos3
		clr.b	v_wtr_routine ; clear water routine counter
		clr.b	f_wtr_state	; clear	water state
		move.b	#1,f_water	; enable water
...
Level_LoadPal:
		...
		cmpi.b	#3,v_act	; is act number 3?
...
Level_GetBgm:
		...
		moveq	#0,d0
		move.b	v_zone,d0
		cmpi.w	#(id_LZ<<8)+3,v_zone ; is level SBZ3?
		bne.s	Level_BgmNotLZ4	; if not, branch
		moveq	#5,d0		; use 5th music (SBZ)

Level_BgmNotLZ4:
		cmpi.w	#(id_SBZ<<8)+2,v_zone ; is level FZ?
...
Level_PlayBgm:
		...
		move.b	#id_TitleCard,v_titlecard ; load title card object

Level_TtlCardLoop:
		move.b	#$C,v_vbla_routine
		bsr.w	WaitForVBla
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
		bsr.w	RunPLC
		...
		tst.l	v_plc_buffer ; are there any items in the pattern load cue?
...
Level_SkipTtlCard:
		...
		move.b	#id_HUD,v_hud ; load HUD object

Level_ChkDebug:
		tst.b	f_debugcheat ; has debug cheat been entered?
		beq.s	Level_ChkWater	; if not, branch
		btst	#bitA,v_jpadhold1 ; is A button held?
		beq.s	Level_ChkWater	; if not, branch
		move.b	#1,f_debugmode ; enable debug mode

Level_ChkWater:
		move.w	#0,v_jpadhold2
		move.w	#0,v_jpadhold1
		cmpi.b	#id_LZ,v_zone ; is level LZ?
		bne.s	Level_LoadObj	; if not, branch
		move.b	#id_WaterSurface,v_watersurface1 ; load water surface object
		move.w	#$60,v_watersurface1+obX
		move.b	#id_WaterSurface,v_watersurface2
		move.w	#$120,v_watersurface2+obX
...
Level_LoadObj:
		...
		jsr	(BuildSprites).l
		moveq	#0,d0
		tst.b	v_lastlamp	; are you starting from	a lamppost?
		bne.s	Level_SkipClr	; if yes, branch
		move.w	d0,v_rings	; clear rings
		move.l	d0,v_time	; clear time
		move.b	d0,v_lifecount ; clear lives counter

Level_SkipClr:
		move.b	d0,f_timeover
		move.b	d0,v_shield	; clear shield
		move.b	d0,v_invinc	; clear invincibility
		move.b	d0,v_shoes	; clear speed shoes
		move.b	d0,v_unused1
		move.w	d0,v_debuguse
		move.w	d0,f_restart
		move.w	d0,v_framecount
		bsr.w	OscillateNumInit
		move.b	#1,f_scorecount ; update score counter
		move.b	#1,f_ringcount ; update rings counter
		move.b	#1,f_timecount ; update time counter
		move.w	#0,v_btnpushtime1
		lea	(DemoDataPtr).l,a1 ; load demo data
		moveq	#0,d0
		move.b	v_zone,d0
		lsl.w	#2,d0
		movea.l	(a1,d0.w),a1
		tst.w	f_demo	; is demo mode on?
...
Level_Demo:
		move.b	1(a1),v_btnpushtime2 ; load key press duration
		subq.b	#1,v_btnpushtime2 ; subtract 1 from duration
		move.w	#1800,v_demolength
		tst.w	f_demo
		bpl.s	Level_ChkWaterPal
		move.w	#540,v_demolength
		cmpi.w	#4,v_creditsnum
		bne.s	Level_ChkWaterPal
		move.w	#510,v_demolength

Level_ChkWaterPal:
		cmpi.b	#id_LZ,v_zone ; is level LZ/SBZ3?
		bne.s	Level_Delay	; if not, branch
		moveq	#palid_LZWater,d0 ; palette $B (LZ underwater)
		cmpi.b	#3,v_act	; is level SBZ3?
...
Level_DelayLoop:
		move.b	#8,v_vbla_routine
		bsr.w	WaitForVBla
		dbf	d1,Level_DelayLoop

		move.w	#$202F,v_pfade_start ; fade in 2nd, 3rd & 4th palette lines
		bsr.w	PalFadeIn_Alt
		tst.w	f_demo	; is an ending sequence demo running?
		bmi.s	Level_ClrCardArt ; if yes, branch
		addq.b	#2,v_ttlcardname+obRoutine ; make title card move
		addq.b	#4,v_ttlcardzone+obRoutine
		addq.b	#4,v_ttlcardact+obRoutine
		addq.b	#4,v_ttlcardoval+obRoutine
...
Level_MainLoop:
		...
		move.b	#8,v_vbla_routine
		bsr.w	WaitForVBla
		addq.w	#1,v_framecount ; add 1 to level timer
...
Level_SkipScroll:
		...
		cmpi.b	#id_Demo,v_gamemode
		beq.s	Level_ChkDemo	; if mode is 8 (demo), branch
		if Revision=0
		tst.w	f_restart	; is the level set to restart?
		bne.w	GM_Level	; if yes, branch
		endif
		cmpi.b	#id_Level,v_gamemode
		beq.w	Level_MainLoop	; if mode is $C (level), branch
		rts	
; ===========================================================================

Level_ChkDemo:
		tst.w	f_restart	; is level set to restart?
...
Level_EndDemo:
		...
		move.b	#id_Sega,v_gamemode ; go to Sega screen
		tst.w	f_demo	; is demo mode on & not ending sequence?
		bpl.s	Level_FadeDemo	; if yes, branch
		move.b	#id_Credits,v_gamemode ; go to credits

Level_FadeDemo:
		move.w	#$3C,v_demolength
...
Level_FDLoop:
		...
		jsr	(BuildSprites).l
		jsr	(ObjPosLoad).l
		subq.w	#1,v_palchgspeed
		bpl.s	loc_3BC8
		move.w	#2,v_palchgspeed
...
ColIndexLoad:
		...
		move.b	v_zone,d0
		lsl.w	#2,d0
		move.l	ColPointers(pc,d0.w),v_collindex
		rts	
...
Sync1:
		subq.b	#1,v_ani0_time ; has timer reached 0?
		bpl.s	Sync2		; if not, branch
		move.b	#$B,v_ani0_time ; reset timer
		subq.b	#1,v_ani0_frame ; next frame
		andi.b	#7,v_ani0_frame ; max frame is 7

; Used for rings and giant rings
Sync2:
		subq.b	#1,v_ani1_time
		bpl.s	Sync3
		move.b	#7,v_ani1_time
		addq.b	#1,v_ani1_frame
		andi.b	#3,v_ani1_frame

; Used for nothing
Sync3:
		subq.b	#1,v_ani2_time
		bpl.s	Sync4
		move.b	#7,v_ani2_time
		addq.b	#1,v_ani2_frame
		cmpi.b	#6,v_ani2_frame
		blo.s	Sync4
		move.b	#0,v_ani2_frame
...
Sync4:
		...
		beq.s	SyncEnd
		moveq	#0,d0
		move.b	v_ani3_time,d0
		add.w	v_ani3_buf,d0
		move.w	d0,v_ani3_buf
		rol.w	#7,d0
		andi.w	#3,d0
		move.b	d0,v_ani3_frame
		subq.b	#1,v_ani3_time
...
SignpostArtLoad:
		...
		cmpi.b	#2,v_act	; is act number 02 (act 3)?
...
GM_Special:
		...
		move.l	#0,v_screenposx
		move.l	#0,v_screenposy
		move.b	#id_SonicSpecial,v_player ; load special stage Sonic object
		bsr.w	PalCycle_SS
		clr.w	v_ssangle	; set stage angle to "upright"
		move.w	#$40,v_ssrotate ; set stage rotation speed
		move.w	#bgm_SS,d0
		...
		lsl.w	#2,d0
		movea.l	(a1,d0.w),a1
		move.b	1(a1),v_btnpushtime2
		subq.b	#1,v_btnpushtime2
		clr.w	v_rings
		clr.b	v_lifecount
		move.w	#0,v_debuguse
		move.w	#1800,v_demolength
		...
		btst	#bitA,v_jpadhold1 ; is A button pressed?
		beq.s	SS_NoDebug	; if not, branch
		move.b	#1,f_debugmode ; enable debug mode
...
SS_MainLoop:
		bsr.w	PauseGame
		move.b	#$A,v_vbla_routine
		bsr.w	WaitForVBla
		bsr.w	MoveSonicInDemo
		move.w	v_jpadhold1,v_jpadhold2
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
		jsr	(SS_ShowLayout).l
		bsr.w	SS_BGAnimate
...
SS_ChkEnd:
		cmpi.b	#id_Special,v_gamemode ; is game mode $10 (special stage)?
		...
		move.b	#id_Level,v_gamemode ; set screen mode to $0C (level)
		cmpi.w	#(id_SBZ<<8)+3,v_zone ; is level number higher than FZ?
		blo.s	SS_Finish	; if not, branch
		clr.w	v_zone	; set to GHZ1

SS_Finish:
		move.w	#60,v_demolength ; set delay time to 1 second
		move.w	#$3F,v_pfade_start
		clr.w	v_palchgspeed
...
loc_47D4:
		...
		move.w	d0,v_ringbonus ; set rings bonus
		move.w	#bgm_GotThrough,d0
		jsr	(PlaySound_Special).l	 ; play end-of-level music

		clearRAM v_objspace,v_objend

		move.b	#id_SSResult,v_ssrescard ; load results screen object
...
SS_ToSegaScreen:
		move.b	#id_Sega,v_gamemode ; goto Sega screen
		rts

		if Revision<>0
SS_ToLevel:	cmpi.b	#id_Level,v_gamemode
...
loc_4992:
		move.w	d0,v_palss_time
		moveq	#0,d0
		move.b	(a0)+,d0
		move.w	d0,v_ssbganim
		lea	(byte_4ABC).l,a1
		lea	(a1,d0.w),a1
		move.w	#$8200,d0
		move.b	(a1)+,d0
		move.w	d0,(a6)
		move.b	(a1),v_scrposy_vdp
		move.w	#$8400,d0
		move.b	(a0)+,d0
		move.w	d0,(a6)
		move.l	#$40000010,(vdp_control_port).l
...
loc_49E8:
		move.w	v_palss_index,d1	; Doesn't seem to ever be modified...
		cmpi.w	#$8A,d0
		blo.s	loc_49F4
		addq.w	#1,d1
...
loc_4C4E:
		cmpi.w	#$C,d0
		bne.s	loc_4C74
		subq.w	#1,v_bg3screenposx
		lea	v_ssscroll_buffer,a3
		move.l	#$18000,d2
		moveq	#7-1,d1
...
GM_Continue:
		...
		move.w	#659,v_demolength ; set time delay to 11 seconds
		clr.l	v_screenposx
		move.l	#$1000000,v_screenposy
		move.b	#id_ContSonic,v_player ; load Sonic object
		move.b	#id_ContScrItem,v_continuetext ; load continue screen objects
		move.b	#id_ContScrItem,v_continuelight
		move.b	#3,v_continuelight+obPriority
		move.b	#4,v_continuelight+obFrame
		move.b	#id_ContScrItem,v_continueicon
...
loc_4DF2:
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
		cmpi.w	#$180,v_player+obX ; has Sonic run off screen?
...
Cont_GotoLevel:
		...
		move.l	d0,v_time	; clear time
		move.l	d0,v_score	; clear score
		move.b	d0,v_lastlamp ; clear lamppost count
		subq.b	#1,v_continues ; subtract 1 from continues
		rts	
; ===========================================================================
...
End_LoadData:
		...
		btst	#bitA,v_jpadhold1 ; is button A pressed?
		beq.s	End_LoadSonic	; if not, branch
		move.b	#1,f_debugmode ; enable debug mode

End_LoadSonic:
		move.b	#id_SonicPlayer,v_player ; load Sonic object
		bset	#0,v_player+obStatus ; make Sonic face left
		move.b	#1,f_lockctrl ; lock controls
		move.w	#(btnL<<8),v_jpadhold2 ; move Sonic to the left
		move.w	#$F800,v_player+obInertia ; set Sonic's speed
		move.b	#id_HUD,v_hud ; load HUD object
		jsr	(ObjPosLoad).l
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
		...
		move.b	#1,f_ringcount
		move.b	#0,f_timecount
		move.w	#1800,v_demolength
...
End_MainLoop:
		...
		bsr.w	OscillateNumDo
		bsr.w	SynchroAnimate
		cmpi.b	#id_Ending,v_gamemode ; is game mode $18 (ending)?
		beq.s	End_ChkEmerald	; if yes, branch

		move.b	#id_Credits,v_gamemode ; goto credits
		move.b	#bgm_Credits,d0
		bsr.w	PlaySound_Special ; play credits music
		move.w	#0,v_creditsnum ; set credits index number to 0
		rts	
; ===========================================================================

End_ChkEmerald:
		tst.w	f_restart	; has Sonic released the emeralds?
...
End_MoveSonic:
		...
		move.w	#(btnR<<8),v_jpadhold2 ; move Sonic to the right
		rts	
; ===========================================================================
...
End_MoveSon2:
		subq.b	#2,d0
		bne.s	End_MoveSon3
		cmpi.w	#$A0,v_player+obX ; has Sonic passed $A0 on x-axis?
		blo.s	End_MoveSonExit	; if not, branch

		addq.b	#2,v_sonicend
		moveq	#0,d0
		move.b	d0,f_lockctrl
		move.w	d0,v_jpadhold2 ; stop Sonic moving
		move.w	d0,v_player+obInertia
		move.b	#$81,f_playerctrl ; lock controls and disable object interaction
		move.b	#fr_Wait2,v_player+obFrame
		move.w	#(id_Wait<<8)+id_Wait,v_player+obAnim ; use "standing" animation
		move.b	#3,v_player+obTimeFrame
		rts	
; ===========================================================================
...
End_MoveSon3:
		...
		addq.b	#2,v_sonicend
		move.w	#$A0,v_player+obX
		move.b	#id_EndSonic,v_player ; load Sonic ending sequence object
		clr.w	v_player+obRoutine
...
GM_Credits:
		...
		moveq	#palid_Sonic,d0
		bsr.w	PalLoad1	; load Sonic's palette
		move.b	#id_CreditsText,v_credits ; load credits object
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
...
Cred_WaitLoop:
		...
		bne.s	Cred_WaitLoop	; if not, branch
		cmpi.w	#9,v_creditsnum ; have the credits finished?
...
EndingDemoLoad:
		...
		move.w	d0,v_zone	; set level from level array
		addq.w	#1,v_creditsnum
		cmpi.w	#9,v_creditsnum ; have credits finished?
		bhs.s	EndDemo_Exit	; if yes, branch
		move.w	#$8001,f_demo ; set demo+ending mode
		move.b	#id_Demo,v_gamemode ; set game mode to 8 (demo)
		move.b	#3,v_lives	; set lives to 3
		moveq	#0,d0
		move.w	d0,v_rings	; clear rings
		move.l	d0,v_time	; clear time
		move.l	d0,v_score	; clear score
		move.b	d0,v_lastlamp ; clear lamppost counter
		cmpi.w	#4,v_creditsnum ; is SLZ demo running?
...
TryAgainEnd:
		...
		moveq	#palid_Ending,d0
		bsr.w	PalLoad1	; load ending palette
		clr.w	v_pal_dry_dup+$40
		move.b	#id_EndEggman,v_endeggman ; load Eggman object
		jsr	(ExecuteObjects).l
		jsr	(BuildSprites).l
		move.w	#1800,v_demolength ; show screen for 30 seconds
		bsr.w	PaletteFadeIn
...
TryAg_MainLoop:
		...
		tst.w	v_demolength ; has 30 seconds elapsed?
		beq.s	TryAg_Exit	; if yes, branch
		cmpi.b	#id_Credits,v_gamemode
...
LoadTilesAsYouMove_BGOnly:
		...
		lea	v_bg2_scroll_flags,a2
		lea	v_bg2screenposx,a3
...
LoadTilesAsYouMove:
		...
		bsr.w	DrawBGScrollBlock1
		lea	v_bg2_scroll_flags_dup,a2	; Scroll block 2 scroll flags
		lea	v_bg2screenposx_dup,a3	; Scroll block 2 X coordinate
		bsr.w	DrawBGScrollBlock2
		if Revision>=1
		; REV01 added a third scroll block, though, technically,
		; the RAM for it was already there in REV00
		lea	v_bg3_scroll_flags_dup,a2	; Scroll block 3 scroll flags
		lea	v_bg3screenposx_dup,a3	; Scroll block 3 X coordinate
		bsr.w	DrawBGScrollBlock3
		endif
		; Then, update the foreground
		lea	v_fg_scroll_flags_dup,a2	; Foreground scroll flags
		lea	v_screenposx_dup,a3		; Foreground X coordinate
		lea	v_lvllayout,a4
...
loc_698E:
		...
		moveq	#-16,d4
		moveq	#-16,d5
		move.w	v_scroll_block_1_size,d6
		move.w	4(a3),d1
...
DrawBGScrollBlock2:
		...
		cmpi.w	#16,(a3)
		blo.s	loc_6A3E
		move.w	v_scroll_block_1_size,d4
		move.w	4(a3),d1
		andi.w	#-16,d1
...
LoadTilesFromStart:
		...
		if Revision<>0
			...
			cmpi.b	#id_MZ,v_zone
			beq.w	Draw_Mz_Bg
			cmpi.w	#(id_SBZ<<8)+0,v_zone
			beq.w	Draw_SBz_Bg
			cmpi.b	#id_EndZ,v_zone
			beq.w	Draw_GHz_Bg
		endif
...
LevelDataLoad:
		...
		addq.l	#4,a2
		movea.l	(a2)+,a0
		lea	v_16x16,a1	; RAM address for 16x16 mappings
		move.w	#0,d0
		bsr.w	EniDec
		movea.l	(a2)+,a0
		if NeoGeo<>0
		...
		move.w	(a2),d0
		andi.w	#$FF,d0
		cmpi.w	#(id_LZ<<8)+3,v_zone ; is level SBZ3 (LZ4) ?
...
LevLoad_ClrRam:
		...
		lea	v_lvllayout,a3 ; RAM address for level layout
		moveq	#0,d1
		bsr.w	LevelLayoutLoad2 ; load	level layout into RAM
		lea	v_lvllayout+$40,a3 ; RAM address for background layout
		moveq	#2,d1
...
ExecuteObjects:
		lea	v_objspace,a0 ; set address for object RAM
...
BuildSprites:
		lea	v_spritetablebuffer,a2 ; set address for sprite table
		moveq	#0,d5
		lea	v_spritequeue,a4
		moveq	#7,d7
...
OPL_Main:
		...
		movea.l	a0,a1
		adda.w	(a0,d0.w),a0
		move.l	a0,v_opl_data
		move.l	a0,v_opl_data+4
		adda.w	2(a1,d0.w),a1
		move.l	a1,v_opl_data+8
		move.l	a1,v_opl_data+$C
		lea	v_objstate,a2
		move.w	#$101,(a2)+
	if FixBugs
...
OPL_Next:
		...
		move.w	v_screenposx,d6
		andi.w	#$FF80,d6
		cmp.w	v_opl_screen,d6
		beq.w	locret_DA3A
		bge.s	loc_D9F6
		move.w	d6,v_opl_screen
		movea.l	v_opl_data+4,a0
		subi.w	#$80,d6
		blo.s	loc_D9D2
...
Sonic_Main:	; Routine 0
		...
		move.w	#$600,v_sonspeedmax ; Sonic's top speed
		move.w	#$C,v_sonspeedacc ; Sonic's acceleration
		move.w	#$80,v_sonspeeddec ; Sonic's deceleration

Sonic_Control:	; Routine 2
		tst.w	f_debugmode	; is debug cheat enabled?
		beq.s	loc_12C58	; if not, branch
		btst	#bitB,v_jpadpress1 ; is button B pressed?
		beq.s	loc_12C58	; if not, branch
		move.w	#1,v_debuguse ; change Sonic into a ring/item
		clr.b	f_lockctrl
		rts	
; ===========================================================================

loc_12C58:
		tst.b	f_lockctrl	; are controls locked?
		bne.s	loc_12C64	; if yes, branch
		move.w	v_jpadhold1,v_jpadhold2 ; enable joypad control

loc_12C64:
		btst	#0,f_playerctrl ; are controls locked?
...
loc_12C7E:
		...
		bsr.w	Sonic_Water
		move.b	v_anglebuffer,objoff_36(a0)
		move.b	v_anglebuffer2,objoff_37(a0)
		tst.b	f_wtunnelmode
		beq.s	loc_12CA6
		tst.b	obAnim(a0)
		bne.s	loc_12CA6
...
ResumeMusic:
		cmpi.w	#12,v_air	; more than 12 seconds of air left?
		bhi.s	.over12		; if yes, branch
		move.w	#bgm_LZ,d0	; play LZ music
		cmpi.w	#(id_LZ<<8)+3,v_zone ; check if level is 0103 (SBZ3)
		bne.s	.notsbz
...
.notinvinc:
			tst.b	f_lockscreen ; is Sonic at a boss?
...
.over12:
		move.w	#30,v_air	; reset air to 30 seconds
		clr.b	v_sonicbubbles+$32
		rts	
...
Sonic_WalkSpeed:
		...
		swap	d2
		swap	d3
		move.b	d0,v_anglebuffer
		move.b	d0,v_anglebuffer2
		move.b	d0,d1
		addi.b	#$20,d0
		bpl.s	loc_14D1A
		move.b	d1,d0
...
loc_1B2C8:
		move.b	v_ani1_frame,$1D0(a1)
		subq.b	#1,v_ani2_time
		bpl.s	loc_1B2E4
		move.b	#7,v_ani2_time
		addq.b	#1,v_ani2_frame
		andi.b	#1,v_ani2_frame

loc_1B2E4:
		move.b	v_ani2_frame,d0
		move.b	d0,$138(a1)
		move.b	d0,$160(a1)
		move.b	d0,$148(a1)
		move.b	d0,$150(a1)
		...
		subq.b	#1,v_ani3_time
		bpl.s	loc_1B326
		move.b	#4,v_ani3_time
		addq.b	#1,v_ani3_frame
		andi.b	#3,v_ani3_frame

loc_1B326:
		move.b	v_ani3_frame,d0
		move.b	d0,$168(a1)
		move.b	d0,$170(a1)
		move.b	d0,$178(a1)
		move.b	d0,$180(a1)
		subq.b	#1,v_ani0_time
		bpl.s	loc_1B350
		move.b	#7,v_ani0_time
		subq.b	#1,v_ani0_frame
		andi.b	#7,v_ani0_frame

loc_1B350:
		lea	((v_ssblocktypes+$16)&$FFFFFF).l,a1
		lea	(SS_WaRiVramSet).l,a0
		moveq	#0,d0
		move.b	v_ani0_frame,d0
		add.w	d0,d0
		lea	(a0,d0.w),a0
		move.w	(a0),(a1)
		move.w	2(a0),8(a1)
...
SS_Load:
		moveq	#0,d0
		move.b	v_lastspecial,d0 ; load number of last special stage entered
		addq.b	#1,v_lastspecial
		cmpi.b	#6,v_lastspecial
		blo.s	SS_ChkEmldNum
		move.b	#0,v_lastspecial ; reset if higher than 6

SS_ChkEmldNum:
		cmpi.b	#6,v_emeralds ; do you have all emeralds?
		beq.s	SS_LoadData	; if yes, branch
		moveq	#0,d1
		move.b	v_emeralds,d1
		subq.b	#1,d1
		blo.s	SS_LoadData
		lea	v_emldlist,a3 ; check which emeralds you have
...
AddPoints:
		move.b	#1,f_scorecount ; set score counter to update
...
.belowmax:
			move.l  (a3),d0
			cmp.l   v_scorelife,d0 ; has Sonic got 50000+ points?
			blo.s   .noextralife ; if not, branch

			addi.l  #5000,v_scorelife ; increase requirement by 50000
			tst.b   v_megadrive
			bmi.s   .noextralife ; branch if Mega Drive is Japanese
			addq.b  #1,v_lives ; give extra life
			addq.b  #1,f_lifecount

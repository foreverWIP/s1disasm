...
.wobble:
		tst.b	f_wtunnelmode ; is Sonic in a water tunnel?
...
Drown_AirLeft:	; Routine $C
		cmpi.w	#$C,v_air	; check air remaining
...
Drown_Countdown:; Routine $A
		...
		bhs.w	.nocountdown
		btst	#6,v_player+obStatus ; is Sonic underwater?
		...
		move.w	v_air,d0	; check air remaining
		cmpi.w	#25,d0
...
.reduceair:
		subq.w	#1,v_air	; subtract 1 from air remaining
...
.makenum:
		...
		move.w	v_player+obX,obX(a1) ; match X position to Sonic
		moveq	#6,d0
		btst	#0,v_player+obStatus
		beq.s	.noflip
		neg.w	d0

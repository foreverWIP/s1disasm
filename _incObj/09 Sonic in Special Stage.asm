...
Obj09_ChkDebug:	; Routine 2
		tst.w	f_debugmode	; is debug mode	cheat enabled?
		beq.s	Obj09_NoDebug	; if not, branch
		btst	#bitB,v_jpadpress1 ; is button B pressed?
		beq.s	Obj09_NoDebug	; if not, branch
		move.w	#1,v_debuguse ; change Sonic into a ring
...
Obj09_ExitStage:
		addi.w	#$40,v_ssrotate
		cmpi.w	#$1800,v_ssrotate
		bne.s	loc_1BBF4
		move.b	#id_Level,v_gamemode

loc_1BBF4:
		cmpi.w	#$3000,v_ssrotate
		blt.s	loc_1BC12
		move.w	#0,v_ssrotate
		move.w	#$4000,v_ssangle
		addq.b	#2,obRoutine(a0)
...
Obj09_GetCont:
		jsr	(CollectRing).l
		cmpi.w	#50,v_rings	; check if you have 50 rings
		blo.s	Obj09_NoCont
		bset	#0,v_lifecount
		bne.s	Obj09_NoCont
		addq.b	#1,v_continues ; add 1 to number of continues
...
Obj09_Get1Up:
		addq.b	#1,v_lives	; add 1 to number of lives
...
Obj09_GetEmer:
		cmpi.b	#6,v_emeralds ; do you have all the emeralds?
		beq.s	Obj09_NoEmer	; if yes, branch
		subi.b	#$3B,d4
		moveq	#0,d0
		move.b	v_emeralds,d0
		lea	v_emldlist,a2
		move.b	d4,(a2,d0.w)
		addq.b	#1,v_emeralds ; add 1 to number of emeralds
...
Obj09_UPblock:
		...
		btst	#6,v_ssrotate+1
		beq.s	Obj09_UPsnd
		asl	v_ssrotate	; increase stage rotation speed
...
Obj09_DOWNblock:
		...
		btst	#6,v_ssrotate+1
		bne.s	Obj09_DOWNsnd
		asr	v_ssrotate	; reduce stage rotation speed
		movea.l	objoff_32(a0),a1
		subq.l	#1,a1
...
Obj09_RevStage:
		neg.w	v_ssrotate	; reverse stage rotation
		move.w	#sfx_SSItem,d0

...
Got_ChkPLC:	; Routine 0
		tst.l	v_plc_buffer ; are the pattern load cues empty?
...
Got_Loop:
		...
		cmpi.b	#6,d0
		bne.s	loc_C5CA
		add.b	v_act,d0	; add act number to frame number
...
loc_C61A:
		cmpi.b	#$E,v_endcardring+obRoutine
		beq.s	loc_C610
		cmpi.b	#4,obFrame(a0)
		bne.s	loc_C5FE
...
Got_TimeBonus:	; Routine 6
		bsr.w	DisplaySprite
		move.b	#1,f_endactbonus ; set time/ring bonus update flag
		moveq	#0,d0
		tst.w	v_timebonus	; is time bonus	= zero?
		beq.s	Got_RingBonus	; if yes, branch
		addi.w	#10,d0		; add 10 to score
		subi.w	#10,v_timebonus ; subtract 10 from time bonus

Got_RingBonus:
		tst.w	v_ringbonus	; is ring bonus	= zero?
		beq.s	Got_ChkBonus	; if yes, branch
		addi.w	#10,d0		; add 10 to score
		subi.w	#10,v_ringbonus ; subtract 10 from ring bonus
...
Got_ChkSS:
		...
		tst.b	f_bigring	; has Sonic jumped into	a giant	ring?
		beq.s	loc_C6EA	; if not, branch
		move.b	#id_Special,v_gamemode ; set game mode to Special Stage (10)
		bra.s	Got_Display2
; ===========================================================================
...
Got_SBZ2:
		...
		bne.w	DeleteObject
		addq.b	#2,obRoutine(a0)
		clr.b	f_lockctrl	; unlock controls
		move.w	#bgm_FZ,d0

; ---------------------------------------------------------------------------
; Dynamic level events
; ---------------------------------------------------------------------------

; ||||||||||||||| S U B	R O U T	I N E |||||||||||||||||||||||||||||||||||||||


DynamicLevelEvents:
		moveq	#0,d0
		move.b	(v_zone).l,d0
		add.w	d0,d0
		move.w	DLE_Index(pc,d0.w),d0
		jsr	DLE_Index(pc,d0.w) ; run level-specific events
		moveq	#2,d1
		move.w	(v_limitbtm1).l,d0
		sub.w	(v_limitbtm2).l,d0 ; has lower level boundary changed recently?
		beq.s	DLE_NoChg	; if not, branch
		bcc.s	loc_6DAC

		neg.w	d1
		move.w	(v_screenposy).l,d0
		cmp.w	(v_limitbtm1).l,d0
		bls.s	loc_6DA0
		move.w	d0,(v_limitbtm2).l
		andi.w	#$FFFE,(v_limitbtm2).l

loc_6DA0:
		add.w	d1,(v_limitbtm2).l
		move.b	#1,(f_bgscrollvert).l

DLE_NoChg:
		rts	
; ===========================================================================

loc_6DAC:
		move.w	(v_screenposy).l,d0
		addq.w	#8,d0
		cmp.w	(v_limitbtm2).l,d0
		blo.s	loc_6DC4
		btst	#1,(v_player+obStatus).l
		beq.s	loc_6DC4
		add.w	d1,d1
		add.w	d1,d1

loc_6DC4:
		add.w	d1,(v_limitbtm2).l
		move.b	#1,(f_bgscrollvert).l
		rts	
; End of function DynamicLevelEvents

; ===========================================================================
; ---------------------------------------------------------------------------
; Offset index for dynamic level events
; ---------------------------------------------------------------------------
DLE_Index:	dc.w DLE_GHZ-DLE_Index, DLE_LZ-DLE_Index
		dc.w DLE_MZ-DLE_Index, DLE_SLZ-DLE_Index
		dc.w DLE_SYZ-DLE_Index, DLE_SBZ-DLE_Index
		zonewarning DLE_Index,2
		dc.w DLE_Ending-DLE_Index
; ===========================================================================
; ---------------------------------------------------------------------------
; Green	Hill Zone dynamic level events
; ---------------------------------------------------------------------------

DLE_GHZ:
		if MMD_Is_GHZ||MMD_Is_Title||MMD_Is_Ending
		moveq	#0,d0
		move.b	(v_act).l,d0
		add.w	d0,d0
		move.w	DLE_GHZx(pc,d0.w),d0
		jmp	DLE_GHZx(pc,d0.w)
; ===========================================================================
DLE_GHZx:	dc.w DLE_GHZ1-DLE_GHZx
		dc.w DLE_GHZ2-DLE_GHZx
		dc.w DLE_GHZ3-DLE_GHZx
; ===========================================================================

DLE_GHZ1:
		move.w	#$300,(v_limitbtm1).l ; set lower y-boundary
		cmpi.w	#$1780,(v_screenposx).l ; has the camera reached $1780 on x-axis?
		blo.s	locret_6E08	; if not, branch
		move.w	#$400,(v_limitbtm1).l ; set lower y-boundary

locret_6E08:
		rts	
; ===========================================================================

DLE_GHZ2:
		move.w	#$300,(v_limitbtm1).l
		cmpi.w	#$ED0,(v_screenposx).l
		blo.s	locret_6E3A
		move.w	#$200,(v_limitbtm1).l
		cmpi.w	#$1600,(v_screenposx).l
		blo.s	locret_6E3A
		move.w	#$400,(v_limitbtm1).l
		cmpi.w	#$1D60,(v_screenposx).l
		blo.s	locret_6E3A
		move.w	#$300,(v_limitbtm1).l

locret_6E3A:
		rts	
; ===========================================================================

DLE_GHZ3:
		moveq	#0,d0
		move.b	(v_dle_routine).l,d0
		move.w	off_6E4A(pc,d0.w),d0
		jmp	off_6E4A(pc,d0.w)
; ===========================================================================
off_6E4A:	dc.w DLE_GHZ3main-off_6E4A
		dc.w DLE_GHZ3boss-off_6E4A
		dc.w DLE_GHZ3end-off_6E4A
; ===========================================================================

DLE_GHZ3main:
		move.w	#$300,(v_limitbtm1).l
		cmpi.w	#$380,(v_screenposx).l
		blo.s	locret_6E96
		move.w	#$310,(v_limitbtm1).l
		cmpi.w	#$960,(v_screenposx).l
		blo.s	locret_6E96
		cmpi.w	#$280,(v_screenposy).l
		blo.s	loc_6E98
		move.w	#$400,(v_limitbtm1).l
		cmpi.w	#$1380,(v_screenposx).l
		bhs.s	loc_6E8E
		move.w	#$4C0,(v_limitbtm1).l
		move.w	#$4C0,(v_limitbtm2).l

loc_6E8E:
		cmpi.w	#$1700,(v_screenposx).l
		bhs.s	loc_6E98

locret_6E96:
		rts	
; ===========================================================================

loc_6E98:
		move.w	#boss_ghz_y,(v_limitbtm1).l
		addq.b	#2,(v_dle_routine).l
		rts	
; ===========================================================================

DLE_GHZ3boss:
		cmpi.w	#$960,(v_screenposx).l
		bhs.s	loc_6EB0
		subq.b	#2,(v_dle_routine).l

loc_6EB0:
		cmpi.w	#boss_ghz_x,(v_screenposx).l
		blo.s	locret_6EE8
		bsr.w	FindFreeObj
		bne.s	loc_6ED0
		_move.b	#id_BossGreenHill,obID(a1) ; load GHZ boss	object
		move.w	#boss_ghz_x+$100,obX(a1)
		move.w	#boss_ghz_y-$80,obY(a1)

loc_6ED0:
		move.w	#bgm_Boss,d0
		bsr.w	PlaySound	; play boss music
		move.b	#1,(f_lockscreen).l ; lock screen
		addq.b	#2,(v_dle_routine).l
		moveq	#plcid_Boss,d0
		bra.w	AddPLC		; load boss patterns
; ===========================================================================

locret_6EE8:
		rts	
; ===========================================================================

DLE_GHZ3end:
		move.w	(v_screenposx).l,(v_limitleft2).l
		endif
		rts	
; ===========================================================================
; ---------------------------------------------------------------------------
; Labyrinth Zone dynamic level events
; ---------------------------------------------------------------------------

DLE_LZ:
		if MMD_Is_LZ
		moveq	#0,d0
		move.b	(v_act).l,d0
		add.w	d0,d0
		move.w	DLE_LZx(pc,d0.w),d0
		jmp	DLE_LZx(pc,d0.w)
; ===========================================================================
DLE_LZx:	dc.w DLE_LZ12-DLE_LZx
		dc.w DLE_LZ12-DLE_LZx
		dc.w DLE_LZ3-DLE_LZx
		dc.w DLE_SBZ3-DLE_LZx
; ===========================================================================

DLE_LZ12:
		rts	
; ===========================================================================

DLE_LZ3:
		tst.b	(f_switch+$F).l	; has switch $F	been pressed?
		beq.s	loc_6F28	; if not, branch
		lea	(v_lvllayout+$80*2+6).l,a1
		cmpi.b	#7,(a1)
		beq.s	loc_6F28
		move.b	#7,(a1)		; modify level layout
		move.w	#sfx_Rumbling,d0
		bsr.w	PlaySound_Special ; play rumbling sound

loc_6F28:
		tst.b	(v_dle_routine).l
		bne.s	locret_6F64
		cmpi.w	#boss_lz_x-$140,(v_screenposx).l
		blo.s	locret_6F62
		cmpi.w	#boss_lz_y+$540,(v_screenposy).l
		bhs.s	locret_6F62
		bsr.w	FindFreeObj
		bne.s	loc_6F4A
		_move.b	#id_BossLabyrinth,obID(a1) ; load LZ boss object

loc_6F4A:
		move.w	#bgm_Boss,d0
		bsr.w	PlaySound	; play boss music
		move.b	#1,(f_lockscreen).l ; lock screen
		addq.b	#2,(v_dle_routine).l
		moveq	#plcid_Boss,d0
		bra.w	AddPLC		; load boss patterns
; ===========================================================================

locret_6F62:
		rts	
; ===========================================================================

locret_6F64:
		rts	
; ===========================================================================

DLE_SBZ3:
		cmpi.w	#$D00,(v_screenposx).l
		blo.s	locret_6F8C
		cmpi.w	#$18,(v_player+obY).l ; has Sonic reached the top of the level?
		bhs.s	locret_6F8C	; if not, branch
		clr.b	(v_lastlamp).l
		move.w	#1,(f_restart).l ; restart level
		move.w	#(id_SBZ<<8)+2,(v_zone).l ; set level number to 0502 (FZ)
		move.b	#1,(f_playerctrl).l ; lock controls
		quitModule

locret_6F8C:
		endif
		rts	
; ===========================================================================
; ---------------------------------------------------------------------------
; Marble Zone dynamic level events
; ---------------------------------------------------------------------------

DLE_MZ:
		if MMD_Is_MZ
		moveq	#0,d0
		move.b	(v_act).l,d0
		add.w	d0,d0
		move.w	DLE_MZx(pc,d0.w),d0
		jmp	DLE_MZx(pc,d0.w)
; ===========================================================================
DLE_MZx:	dc.w DLE_MZ1-DLE_MZx
		dc.w DLE_MZ2-DLE_MZx
		dc.w DLE_MZ3-DLE_MZx
; ===========================================================================

DLE_MZ1:
		moveq	#0,d0
		move.b	(v_dle_routine).l,d0
		move.w	off_6FB2(pc,d0.w),d0
		jmp	off_6FB2(pc,d0.w)
; ===========================================================================
off_6FB2:	dc.w loc_6FBA-off_6FB2
		dc.w loc_6FEA-off_6FB2
		dc.w loc_702E-off_6FB2
		dc.w loc_7050-off_6FB2
; ===========================================================================

loc_6FBA:
		move.w	#$1D0,(v_limitbtm1).l
		cmpi.w	#$700,(v_screenposx).l
		blo.s	locret_6FE8
		move.w	#$220,(v_limitbtm1).l
		cmpi.w	#$D00,(v_screenposx).l
		blo.s	locret_6FE8
		move.w	#$340,(v_limitbtm1).l
		cmpi.w	#$340,(v_screenposy).l
		blo.s	locret_6FE8
		addq.b	#2,(v_dle_routine).l

locret_6FE8:
		rts	
; ===========================================================================

loc_6FEA:
		cmpi.w	#$340,(v_screenposy).l
		bhs.s	loc_6FF8
		subq.b	#2,(v_dle_routine).l
		rts	
; ===========================================================================

loc_6FF8:
		move.w	#0,(v_limittop2).l
		cmpi.w	#$E00,(v_screenposx).l
		bhs.s	locret_702C
		move.w	#$340,(v_limittop2).l
		move.w	#$340,(v_limitbtm1).l
		cmpi.w	#$A90,(v_screenposx).l
		bhs.s	locret_702C
		move.w	#$500,(v_limitbtm1).l
		cmpi.w	#$370,(v_screenposy).l
		blo.s	locret_702C
		addq.b	#2,(v_dle_routine).l

locret_702C:
		rts	
; ===========================================================================

loc_702E:
		cmpi.w	#$370,(v_screenposy).l
		bhs.s	loc_703C
		subq.b	#2,(v_dle_routine).l
		rts	
; ===========================================================================

loc_703C:
		cmpi.w	#$500,(v_screenposy).l
		blo.s	locret_704E
		if Revision<>0
			cmpi.w	#$B80,(v_screenposx).l
			bcs.s	locret_704E
		endif
		move.w	#$500,(v_limittop2).l
		addq.b	#2,(v_dle_routine).l

locret_704E:
		rts	
; ===========================================================================

loc_7050:
		if Revision<>0
			cmpi.w	#$B80,(v_screenposx).l
			bcc.s	locj_76B8
			cmpi.w	#$340,(v_limittop2).l
			beq.s	locret_7072
			subq.w	#2,(v_limittop2).l
			rts
locj_76B8:
			cmpi.w	#$500,(v_limittop2).l
			beq.s	locj_76CE
			cmpi.w	#$500,(v_screenposy).l
			bcs.s	locret_7072
			move.w	#$500,(v_limittop2).l
locj_76CE:
		endif

		cmpi.w	#$E70,(v_screenposx).l
		blo.s	locret_7072
		move.w	#0,(v_limittop2).l
		move.w	#$500,(v_limitbtm1).l
		cmpi.w	#$1430,(v_screenposx).l
		blo.s	locret_7072
		move.w	#$210,(v_limitbtm1).l

locret_7072:
		rts	
; ===========================================================================

DLE_MZ2:
		move.w	#$520,(v_limitbtm1).l
		cmpi.w	#$1700,(v_screenposx).l
		blo.s	locret_7088
		move.w	#$200,(v_limitbtm1).l

locret_7088:
		rts	
; ===========================================================================

DLE_MZ3:
		moveq	#0,d0
		move.b	(v_dle_routine).l,d0
		move.w	off_7098(pc,d0.w),d0
		jmp	off_7098(pc,d0.w)
; ===========================================================================
off_7098:	dc.w DLE_MZ3boss-off_7098
		dc.w DLE_MZ3end-off_7098
; ===========================================================================

DLE_MZ3boss:
		move.w	#$720,(v_limitbtm1).l
		cmpi.w	#boss_mz_x-$2A0,(v_screenposx).l
		blo.s	locret_70E8
		move.w	#boss_mz_y,(v_limitbtm1).l
		cmpi.w	#boss_mz_x-$10,(v_screenposx).l
		blo.s	locret_70E8
		bsr.w	FindFreeObj
		bne.s	loc_70D0
		_move.b	#id_BossMarble,obID(a1) ; load MZ boss object
		move.w	#boss_mz_x+$1F0,obX(a1)
		move.w	#boss_mz_y+$1C,obY(a1)

loc_70D0:
		move.w	#bgm_Boss,d0
		bsr.w	PlaySound	; play boss music
		move.b	#1,(f_lockscreen).l ; lock screen
		addq.b	#2,(v_dle_routine).l
		moveq	#plcid_Boss,d0
		bra.w	AddPLC		; load boss patterns
; ===========================================================================

locret_70E8:
		rts	
; ===========================================================================

DLE_MZ3end:
		move.w	(v_screenposx).l,(v_limitleft2).l
		endif
		rts	
; ===========================================================================
; ---------------------------------------------------------------------------
; Star Light Zone dynamic level events
; ---------------------------------------------------------------------------

DLE_SLZ:
		if MMD_Is_SLZ
		moveq	#0,d0
		move.b	(v_act).l,d0
		add.w	d0,d0
		move.w	DLE_SLZx(pc,d0.w),d0
		jmp	DLE_SLZx(pc,d0.w)
; ===========================================================================
DLE_SLZx:	dc.w DLE_SLZ12-DLE_SLZx
		dc.w DLE_SLZ12-DLE_SLZx
		dc.w DLE_SLZ3-DLE_SLZx
; ===========================================================================

DLE_SLZ12:
		rts	
; ===========================================================================

DLE_SLZ3:
		moveq	#0,d0
		move.b	(v_dle_routine).l,d0
		move.w	off_7118(pc,d0.w),d0
		jmp	off_7118(pc,d0.w)
; ===========================================================================
off_7118:	dc.w DLE_SLZ3main-off_7118
		dc.w DLE_SLZ3boss-off_7118
		dc.w DLE_SLZ3end-off_7118
; ===========================================================================

DLE_SLZ3main:
		cmpi.w	#boss_slz_x-$190,(v_screenposx).l
		blo.s	locret_7130
		move.w	#boss_slz_y,(v_limitbtm1).l
		addq.b	#2,(v_dle_routine).l

locret_7130:
		rts	
; ===========================================================================

DLE_SLZ3boss:
		cmpi.w	#boss_slz_x,(v_screenposx).l
		blo.s	locret_715C
		bsr.w	FindFreeObj
		bne.s	loc_7144
		move.b	#id_BossStarLight,obID(a1) ; load SLZ boss object

loc_7144:
		move.w	#bgm_Boss,d0
		bsr.w	PlaySound	; play boss music
		move.b	#1,(f_lockscreen).l ; lock screen
		addq.b	#2,(v_dle_routine).l
		moveq	#plcid_Boss,d0
		bra.w	AddPLC		; load boss patterns
; ===========================================================================

locret_715C:
		rts	
; ===========================================================================

DLE_SLZ3end:
		move.w	(v_screenposx).l,(v_limitleft2).l
		endif
		rts
; ===========================================================================
; ---------------------------------------------------------------------------
; Spring Yard Zone dynamic level events
; ---------------------------------------------------------------------------

DLE_SYZ:
		if MMD_Is_SYZ
		moveq	#0,d0
		move.b	(v_act).l,d0
		add.w	d0,d0
		move.w	DLE_SYZx(pc,d0.w),d0
		jmp	DLE_SYZx(pc,d0.w)
; ===========================================================================
DLE_SYZx:	dc.w DLE_SYZ1-DLE_SYZx
		dc.w DLE_SYZ2-DLE_SYZx
		dc.w DLE_SYZ3-DLE_SYZx
; ===========================================================================

DLE_SYZ1:
		rts	
; ===========================================================================

DLE_SYZ2:
		move.w	#$520,(v_limitbtm1).l
		cmpi.w	#$25A0,(v_screenposx).l
		blo.s	locret_71A2
		move.w	#$420,(v_limitbtm1).l
		cmpi.w	#$4D0,(v_player+obY).l
		blo.s	locret_71A2
		move.w	#$520,(v_limitbtm1).l

locret_71A2:
		rts	
; ===========================================================================

DLE_SYZ3:
		moveq	#0,d0
		move.b	(v_dle_routine).l,d0
		move.w	off_71B2(pc,d0.w),d0
		jmp	off_71B2(pc,d0.w)
; ===========================================================================
off_71B2:	dc.w DLE_SYZ3main-off_71B2
		dc.w DLE_SYZ3boss-off_71B2
		dc.w DLE_SYZ3end-off_71B2
; ===========================================================================

DLE_SYZ3main:
		cmpi.w	#boss_syz_x-$140,(v_screenposx).l
		blo.s	locret_71CE
		bsr.w	FindFreeObj
		bne.s	locret_71CE
		move.b	#id_BossBlock,obID(a1) ; load blocks that boss picks up
		addq.b	#2,(v_dle_routine).l

locret_71CE:
		rts	
; ===========================================================================

DLE_SYZ3boss:
		cmpi.w	#boss_syz_x,(v_screenposx).l
		blo.s	locret_7200
		move.w	#boss_syz_y,(v_limitbtm1).l
		bsr.w	FindFreeObj
		bne.s	loc_71EC
		move.b	#id_BossSpringYard,obID(a1) ; load SYZ boss object
		addq.b	#2,(v_dle_routine).l

loc_71EC:
		move.w	#bgm_Boss,d0
		bsr.w	PlaySound	; play boss music
		move.b	#1,(f_lockscreen).l ; lock screen
		moveq	#plcid_Boss,d0
		bra.w	AddPLC		; load boss patterns
; ===========================================================================

locret_7200:
		rts	
; ===========================================================================

DLE_SYZ3end:
		move.w	(v_screenposx).l,(v_limitleft2).l
		endif
		rts	
; ===========================================================================
; ---------------------------------------------------------------------------
; Scrap	Brain Zone dynamic level events
; ---------------------------------------------------------------------------

DLE_SBZ:
		if MMD_Is_SBZ
		moveq	#0,d0
		move.b	(v_act).l,d0
		add.w	d0,d0
		move.w	DLE_SBZx(pc,d0.w),d0
		jmp	DLE_SBZx(pc,d0.w)
; ===========================================================================
DLE_SBZx:	dc.w DLE_SBZ1-DLE_SBZx
		dc.w DLE_SBZ2-DLE_SBZx
		dc.w DLE_FZ-DLE_SBZx
; ===========================================================================

DLE_SBZ1:
		move.w	#$720,(v_limitbtm1).l
		cmpi.w	#$1880,(v_screenposx).l
		blo.s	locret_7242
		move.w	#$620,(v_limitbtm1).l
		cmpi.w	#$2000,(v_screenposx).l
		blo.s	locret_7242
		move.w	#$2A0,(v_limitbtm1).l

locret_7242:
		rts	
; ===========================================================================

DLE_SBZ2:
		moveq	#0,d0
		move.b	(v_dle_routine).l,d0
		move.w	off_7252(pc,d0.w),d0
		jmp	off_7252(pc,d0.w)
; ===========================================================================
off_7252:	dc.w DLE_SBZ2main-off_7252
		dc.w DLE_SBZ2boss-off_7252
		dc.w DLE_SBZ2boss2-off_7252
		dc.w DLE_SBZ2end-off_7252
; ===========================================================================

DLE_SBZ2main:
		move.w	#$800,(v_limitbtm1).l
		cmpi.w	#$1800,(v_screenposx).l
		blo.s	locret_727A
		move.w	#boss_sbz2_y,(v_limitbtm1).l
		cmpi.w	#$1E00,(v_screenposx).l
		blo.s	locret_727A
		addq.b	#2,(v_dle_routine).l

locret_727A:
		rts	
; ===========================================================================

DLE_SBZ2boss:
		cmpi.w	#boss_sbz2_x-$1A0,(v_screenposx).l
		blo.s	locret_7298
		bsr.w	FindFreeObj
		bne.s	locret_7298
		move.b	#id_FalseFloor,obID(a1) ; load collapsing block object
		addq.b	#2,(v_dle_routine).l
		moveq	#plcid_EggmanSBZ2,d0
		bra.w	AddPLC		; load SBZ2 Eggman patterns
; ===========================================================================

locret_7298:
		rts	
; ===========================================================================

DLE_SBZ2boss2:
		cmpi.w	#boss_sbz2_x-$F0,(v_screenposx).l
		blo.s	loc_72B6
		bsr.w	FindFreeObj
		bne.s	loc_72B0
		move.b	#id_ScrapEggman,obID(a1) ; load SBZ2 Eggman object
		addq.b	#2,(v_dle_routine).l

loc_72B0:
		move.b	#1,(f_lockscreen).l ; lock screen

loc_72B6:
		bra.s	loc_72C2
; ===========================================================================

DLE_SBZ2end:
		cmpi.w	#boss_sbz2_x,(v_screenposx).l
		blo.s	loc_72C2
		rts	
; ===========================================================================

loc_72C2:
		move.w	(v_screenposx).l,(v_limitleft2).l
		rts	
; ===========================================================================

DLE_FZ:
		moveq	#0,d0
		move.b	(v_dle_routine).l,d0
		move.w	off_72D8(pc,d0.w),d0
		jmp	off_72D8(pc,d0.w)
; ===========================================================================
off_72D8:	dc.w DLE_FZmain-off_72D8, DLE_FZboss-off_72D8
		dc.w DLE_FZend-off_72D8, locret_7322-off_72D8
		dc.w DLE_FZend2-off_72D8
; ===========================================================================

DLE_FZmain:
		cmpi.w	#boss_fz_x-$308,(v_screenposx).l
		blo.s	loc_72F4
		addq.b	#2,(v_dle_routine).l
		moveq	#plcid_FZBoss,d0
		bsr.w	AddPLC		; load FZ boss patterns

loc_72F4:
		bra.s	loc_72C2
; ===========================================================================

DLE_FZboss:
		cmpi.w	#boss_fz_x-$150,(v_screenposx).l
		blo.s	loc_7312
		bsr.w	FindFreeObj
		bne.s	loc_7312
		move.b	#id_BossFinal,obID(a1) ; load FZ boss object
		addq.b	#2,(v_dle_routine).l
		move.b	#1,(f_lockscreen).l ; lock screen

loc_7312:
		bra.s	loc_72C2
; ===========================================================================

DLE_FZend:
		cmpi.w	#boss_fz_x,(v_screenposx).l
		blo.s	loc_7320
		addq.b	#2,(v_dle_routine).l

loc_7320:
		bra.s	loc_72C2
; ===========================================================================

locret_7322:
		rts	
; ===========================================================================

DLE_FZend2:
		bra.s	loc_72C2
		else
		rts
		endif
; ===========================================================================
; ---------------------------------------------------------------------------
; Ending sequence dynamic level events (empty)
; ---------------------------------------------------------------------------

DLE_Ending:
		rts	

...
PaletteCycle:
		moveq	#0,d2
		moveq	#0,d0
		move.b	v_zone,d0	; get level number
		add.w	d0,d0
...
PCycGHZ_Go:
		subq.w	#1,v_pcyc_time ; decrement timer
		bpl.s	PCycGHZ_Skip	; if time remains, branch

		move.w	#5,v_pcyc_time ; reset timer to 5 frames
		move.w	v_pcyc_num,d0 ; get cycle number
		addq.w	#1,v_pcyc_num ; increment cycle number
...
; Waterfalls
		...
		move.w	#2,v_pcyc_time ; reset timer to 2 frames
		move.w	v_pcyc_num,d0
		addq.w	#1,v_pcyc_num ; increment cycle number
		andi.w	#3,d0		; if cycle > 3, reset to 0
		lsl.w	#3,d0
		lea	(Pal_LZCyc1).l,a0
		cmpi.b	#3,v_act	; check if level is SBZ3
...
; Conveyor belts
		...
		moveq	#1,d1
		tst.b	f_conveyrev	; have conveyor belts been reversed?
		beq.s	PCycLZ_NoRev	; if not, branch
		neg.w	d1

PCycLZ_NoRev:
		move.w	v_pal_buffer,d0
		andi.w	#3,d0
		add.w	d1,d0
		cmpi.w	#3,d0
		blo.s	loc_1A0A
